...
function Report() {
  const { currency, analysisDate, selectData, currencyRatio, currencyFixed } = useContext(DataContext);
  ...
  const fetchData = useCallback(async (pageIndex: number, pageSize: number, sorting: Sorting) => {
    ...
    try {
      ...
      `);
      if (countTemp) {
        setPageCount(Math.ceil(countTemp[0].total as number / pageSize));
      }
  ...
  const handleExport = async () => {
    ...
    const timer = setInterval(async () => {
      if (idx === times) {
        ...
        worksheet.getRow(1).values = ['Date', 'Unit ID', 'Request', 'Impression', 'Revenue', 'eCPM', 'RPM', 'Revenue_', 'eCPM_', 'RPM_'];
      ...
      const tableTemp = await selectData(`
        WITH calculated_units AS (
          SELECT 
            date, unitId, impression, request,
            ROUND(revenue * ${currencyRatio}, ${currencyFixed}) AS revenue,
            (revenue * ${currencyRatio}) AS revenue_
          FROM tbl_unit_reports
          WHERE date BETWEEN '${analysisDate[0]}' AND '${analysisDate[1]}'
          ${searchTxt !== '' ? `AND unitId LIKE '%${searchTxt}%'` : ''}
        )
        SELECT date, unitId, impression, request, revenue, revenue_,
        ROUND(CASE WHEN impression = 0 THEN 0 ELSE ((revenue / impression) * 1000) END, ${currencyFixed}) AS ecpm,
        ROUND(CASE WHEN request = 0 THEN 0 ELSE ((revenue / request) * 1000) END, ${currencyFixed}) AS rpm,
        CASE WHEN impression = 0 THEN 0 ELSE ((revenue_ / impression) * 1000) END AS ecpm_,
        CASE WHEN request = 0 THEN 0 ELSE ((revenue_ / request) * 1000) END AS rpm_
        FROM calculated_units
        ORDER BY ${sorting.id} ${sorting.desc ? 'DESC' : 'ASC'}
  ...
  return (
    <div>
      ...
      {
        isExporting && (
          <div className='w-full h-full bg-white/60 absolute top-0 left-0 flex items-center justify-center'>
